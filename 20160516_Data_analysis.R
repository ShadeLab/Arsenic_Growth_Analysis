...
#read in raw data
data=data.frame(read.table("20160516_01_AsIII_I06,7,26,27,42,45,A06.asc"), row.names=TRUE)
...
neg0=data.frame(rep(data.frame(data[,58]), 12))
neg1=data.frame(rep(data.frame(data[,70]), 12))
neg3=data.frame(rep(data.frame(data[,82]), 12))
neg5=data.frame(rep(data.frame(data[,94]), 12))
neg=data.frame(cbind(neg0,neg1,neg3,neg5,neg0,neg1,neg3,neg5))
data= data - neg
...
#read in platemap
platemap=read.csv("20160516_Platemap_MIC.csv")
...
#remove rows that had no inoculum 
data=data[!(data$Strain=="x"),]

#make time file match size of data file
time=time[-(1:8),]
...
#save results table
write.csv(results, "20160516_results")

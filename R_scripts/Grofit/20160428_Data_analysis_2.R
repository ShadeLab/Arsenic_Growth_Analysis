...
#read in raw data
data=data.frame(read.table("20160428_AsV_MIC_A08,27,31,I16.asc"), row.names=TRUE)
...
#read in platemap
platemap=read.csv("20160428_Platemap_High_MIC.csv")
...
#remove rows with negative data (this set only!)
data2=data[-47,]
time2=time[-47,]
...
results=gcFit(time2, data2, control)

#make a data frame of the results
results=data.frame(summary(results))

#save results table
write.csv(results, "20160428_results")
...
#save EC50 results
write.csv(EC50, "20160428_EC50")

#extract maximum growth parameter from dataset
mu=data.frame(results$mu.spline, results$reliability)

#remove bad well from platemap
platemap2=platemap[-47,]

#add information to mu
mu=cbind(platemap2, mu)

#add replicate number to data
reps=data.frame(read.table("20160428_replicates.csv", header=TRUE))
mu=cbind(mu, reps)

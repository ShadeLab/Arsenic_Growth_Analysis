...
#read in raw data
data=data.frame(read.table("20160524_AsV_A27,I45,47,49.asc"), row.names=TRUE)
...
neg300=data.frame(cbind(neg300, data[,34], data[,58]))
...
#read in platemap
platemap=read.csv("20160523_platemap.csv")
...
#remove I2747 data because it is irregular and was repeated
results=results[!(results$TestId=="I2747"),]


#save results table
write.csv(results, "20160523_results")
...
reps=data.frame(read.table("20160523_replicates.csv", header=TRUE))
mu=cbind(mu, reps)

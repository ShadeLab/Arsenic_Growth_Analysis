...
#read in raw data
data=data.frame(read.table("20160531_AsV_AsIII_I23,46,47,49,59.asc"), row.names=TRUE)
...
neg0=data.frame(data[,7])
neg10=data.frame(data[,8])
neg50=data.frame(data[,9])
neg100=data.frame(data[,10])
neg150=data.frame(data[,11])
neg200=data.frame(data[,12])
negV=data.frame(cbind(neg0,neg10,neg50,neg100,neg150,neg200))
neg=data.frame(rep(negV,9))
neg0=data.frame(data[,55])
neg1=data.frame(data[,56])
neg3=data.frame(data[,57])
neg5=data.frame(data[,58])
...
#save results table
write.csv(results, "20160531_results")
...
#save EC50 results
write.csv(EC50, "20160531_EC50")

#extract maximum growth parameter from dataset
mu=data.frame(results$mu.spline, results$reliability)

#read updated platemap (does not include wells with only <0 values)
platemap=data.frame(read.csv("20160531_platemap_grofit.csv"))

#add information to mu
mu=cbind(platemap, mu)

#add replicate number to data
reps=data.frame(read.table("20160531_replicates.csv", header=TRUE))
mu=cbind(mu, reps)
...
ggplot(data=stats, aes(x=Concentration, y=Average)) +
  ...
  facet_wrap(Arsenic~Strain, scales="free_x")
...
#group data
grouped=group_by(int, Arsenic, Strain, Concentration)
